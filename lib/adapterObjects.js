...
async function setAdvertisementData(adapter, data) {
    ...
    if (data.serviceData) {
        ...
        if (data.serviceData.model === 'H') {
            ...
            // SwitchBot "Meter" (Plus) and "Indoor/Outdoor Thermo-Hygrometer"
            let offsetTemperature = await adapter.getStateAsync(data.address + '.offset.temperature').value.toFixed(1);
            let offsetTemperatureF = await adapter.getStateAsync(data.address + '.offset.temperatureF').value.toFixed(1);
            let offsetHumidity = await adapter.getStateAsync(data.address + '.offset.humidity').value.toFixed(0);
