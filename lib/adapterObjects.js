 ...
 *                        movement: Boolean, doorState: String}}} data
 */
async function createBotObjects(adapter, data) {
    ...
    if (data.serviceData.model === 'H') {
        ...
        // Check if temperature data exists before accessing it
        const tempC = data.serviceData.temperature && data.serviceData.temperature.c ? data.serviceData.temperature.c : 0;
        const tempF = data.serviceData.temperature && data.serviceData.temperature.f ? data.serviceData.temperature.f : 32;
    ...
    if (data.serviceData) {
        ...
        if (data.serviceData.model === 'H') {
            ...
            if (data.serviceData.temperature && typeof data.serviceData.temperature === 'object') {
                ...
                const offsetTemperature = Number(state.val).toFixed(1);
                state = await adapter.getStateAsync(data.address + '.offset.temperatureF');
                const offsetTemperatureF = Number(state.val).toFixed(1);
                state = await adapter.getStateAsync(data.address + '.offset.humidity');
                const offsetHumidity = Number(state.val).toFixed(0);
                ...
                const temperature = data.serviceData.temperature.c + Number(offsetTemperature);
                const temperatureF = data.serviceData.temperature.f + Number(offsetTemperatureF);
